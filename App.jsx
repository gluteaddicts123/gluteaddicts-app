...
import { Card, Badge, Btn, Input, Modal, Pill, Avatar, ProgressBar, CouponInput, Toast } from './components.jsx';
    ...
    <div style={{ paddingBottom: 20 }}>
      ...
      <div style={{ padding: '0 16px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, margin: '20px 0' }}>
          {[{ icon: '🔥', val: '60', lbl: 'Minutos' }, { icon: '💪', val: '2–3x', lbl: 'Por semana' }, { icon: '📍', val: 'Cra 35', lbl: 'El Poblado' }].map(s => (
            <Card key={s.val} style={{ textAlign: 'center', padding: '14px 8px' }}>
              <div style={{ fontSize: 22 }}>{s.icon}</div>
              <div style={{ fontSize: 18, fontWeight: 900, color: C.gold }}>{s.val}</div>
          ...
          <Card key={pkg.id} style={{ marginBottom: 10, position: 'relative', overflow: 'hidden' }} onClick={() => setScreen('packages')}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <div style={{ textAlign: 'right' }}>
                <div style={{ fontWeight: 900, color: pkg.color, fontSize: 15 }}>{fmt(calcTotal(pkg.basePrice))}</div>
                <div style={{ fontSize: 10, color: C.gray }}>c/IVA</div>
              </div>
            </div>
          </Card>
        ))}
...
function ScheduleScreen() {
  ...
  async function reserve(slot) {
    ...
    if (!cpId) { setBookingKey(key); return; } // Show modal to select package
    setLoading(true);
    try {
      await API.createBooking({
        customer_package_id: parseInt(cpId),
        service_name: type === 'jumping' ? 'Solo Jumping' : 'Glúteos + Jumping',
        booking_date: '2026-06-01', // In production: compute from day
        booking_time: slot,
      });
  ...
  return (
    <div style={{ padding: '20px 16px' }}>
      ...
      <div style={{ fontSize: 13, color: C.gray, marginBottom: 20 }}>Lunes a Domingo · Mañana & Tarde</div>
...
function PackagesScreen({ setScreen, setCheckoutPkg }) {
  return (
    <div style={{ padding: '20px 16px' }}>
      ...
      <div style={{ fontSize: 13, color: C.gray, marginBottom: 6 }}>Todos los precios incluyen IVA (19%)</div>
          ...
          <div key={pkg.id} style={{ marginBottom: 18, position: 'relative' }}>
            ...
            <Card style={{ border: `1.5px solid ${pkg.popular ? C.pink : pkg.id === 3 ? C.purple : C.border}` }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 14 }}>
                ...
                <div style={{ textAlign: 'right' }}>
                  <div style={{ fontSize: 11, color: C.grayD, textDecoration: 'line-through' }}>{fmt(pkg.basePrice)}</div>
                  <div style={{ fontSize: 22, fontWeight: 900, color: pkg.color }}>{fmt(total)}</div>
                  <div style={{ fontSize: 10, color: C.gray }}>incl. IVA 19%</div>
                  {!pkg.unlimited && <div style={{ fontSize: 10, color: pkg.color, fontWeight: 700 }}>{fmt(total / pkg.sessions)}/clase</div>}
                </div>
              </div>
      ...
      {step === 1 && <>
        <Card style={{ marginBottom: 14 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <div style={{ textAlign: 'right' }}>
              <div style={{ fontSize: 11, color: C.grayD, textDecoration: 'line-through' }}>{fmt(base)}</div>
              <div style={{ fontSize: 22, fontWeight: 900, color: pkg.color }}>{fmt(withIva)}</div>
              <div style={{ fontSize: 10, color: C.gray }}>incl. IVA 19%</div>
            </div>
          </div>
        </Card>
        ...
        <div style={{ background: C.surface, borderRadius: 12, padding: 14, marginBottom: 16 }}>
          {[['Precio base', fmt(base)], ['IVA 19%', fmt(iva)], coupon && [`Descuento (${coupon.code})`, `-${fmt(discAmt)}`]].filter(Boolean).map(([k, v], i) => (
            <div key={i} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, marginBottom: 7 }}>
              <span style={{ color: C.gray }}>{k}</span>
              <span style={{ color: i === 2 ? C.success : C.grayL }}>{v}</span>
            </div>
          ))}
          <div style={{ height: 1, background: C.border, margin: '10px 0' }} />
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            <span style={{ fontWeight: 800, color: C.white }}>Total</span>
  ...
  async function submit() {
    ...
    try {
      if (mode === 'login') {
        ...
        await register(regName, email, pass);
      }
    } catch (e) {
    ...
    <div style={{ padding: '20px 16px' }}>
      ...
      <Card style={{ marginTop: 18, textAlign: 'center' }}>
        <div style={{ fontSize: 11, color: C.gray, lineHeight: 2.2 }}>
          <div>📍 Cra. 35 #7-86, El Poblado, Medellín</div>
  ...
  return (
    <div style={{ maxWidth: 430, margin: '0 auto', background: C.black, minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ background: C.dark, padding: 'max(12px, env(safe-area-inset-top)) 20px 10px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: `1px solid ${C.border}`, position: 'sticky', top: 0, zIndex: 200 }}>
        <img src="https://gluteaddictsmedellin.co/wp-content/uploads/2026/01/Logo-2.png" alt="Glute Addicts" style={{ height: 28, objectFit: 'contain', filter: 'brightness(0) invert(1)' }} />
